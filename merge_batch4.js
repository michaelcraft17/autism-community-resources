...
const newPath = path.join(__dirname, 'new_autism_resources_batch4.json');
...
for (const entry of newEntries) {
  ...
  if (!existingNames.has(key)) { existing.push(entry); existingNames.add(key); added++; console.log(`✅ Added: ${entry.name}`); }
  else { console.log(`⏭️  Skipped: ${entry.name}`); }
}
